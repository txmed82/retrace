...
Retries (2 on timeout / 5xx, exponential backoff), a reusable `httpx.Client` (instead of per-call construction), and an `LLMError` exception with strict dict-type guards were added per design spec section 4.
