...
def test_sentry_source_map_lookup_trims_dist(tmp_path: Path) -> None:
    store, workspace = _store(tmp_path)
    upload_source_map(
        store=store,
        project_id=workspace.project_id,
    ...
    result = ingest_monitoring_webhook(
        ...
        payload={
            "event": {
                "event_id": "evt-sourcemap-dist-trim-1",
                "title": "TypeError: failed checkout",
                "release": "abc123",
                "dist": " beta ",
                "exception": {
...
def test_sentry_source_map_miss_records_artifact_diagnostics(tmp_path: Path) -> None:
    ...
    upload_source_map(
        ...
        environment_id=workspace.environment_id,
        release="abc123",
        artifact_url="https://cdn.example.com/assets/other.min.js?token=secret#bundle",
    ...
    assert "token=secret" not in json.dumps(frame["source_map_diagnostic"])
